...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_6y_v30_close_25_control_currentdata");
const jsonPath = path.join(outDir, "nxt_v30_close_25_6y_control_currentdata_results.json");
const xlsxPath = path.join(outDir, "NXT_V30_Close_25_ATR_6Y_Control_CurrentData_BTC_SOL_SUI.xlsx");
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT v3.0 Control With Volume Filter - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | Full exit at ${closeAtr} x ATR | Source: ${data.source} | Net of fee/slippage model`);
