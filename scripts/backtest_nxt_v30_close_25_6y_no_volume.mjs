...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_6y_v30_close_25_no_volume");
const jsonPath = path.join(outDir, "nxt_v30_close_25_6y_no_volume_results.json");
const xlsxPath = path.join(outDir, "NXT_V30_Close_25_ATR_6Y_No_Volume_BTC_SOL_SUI.xlsx");
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT v3.0 No Volume Filter - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | Full exit at ${closeAtr} x ATR | Source: ${data.source} | Net of fee/slippage model`);
...
const data = {
  ...
  assumptions: [
    ...
    "Long entry requires SSL 10/10 bullish crossover, price crossing above EMA20 within the last 3 candles, and distance from close to EMA50 <= 2 ATR(14). Net Volume filter is disabled in this variant.",
    "Short entry requires SSL 10/10 bearish crossover, price crossing below EMA20 within the last 3 candles, and distance from close to EMA50 <= 2 ATR(14). Net Volume filter is disabled in this variant.",
