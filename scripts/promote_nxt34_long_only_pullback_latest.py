...
from test_nxt33_long_only_pullback_continuation import (
...
LATEST_JSON = LATEST / "NXT_Latest_NXT34_Native1D_SSL14_RunnerA_LongOnlyPullbackContinuation_NoRiskOff_6Y_BTC_SOL_SUI_20K.json"
LATEST_XLSX = LATEST / "NXT_Latest_NXT34_Native1D_SSL14_RunnerA_LongOnlyPullbackContinuation_NoRiskOff_6Y_BTC_SOL_SUI_20K.xlsx"
LATEST_DOCX = LATEST / "NXT_Latest_NXT34_System_And_Indicators.docx"
LATEST_SUMMARY = LATEST / "NXT_Latest_Summary.md"

SYSTEM_VERSION = "NXT v3.4 Simple + Binance Native 1D + SSL14 + Runner A + Anti-Immediate-Reversal + LONG-only Pullback Continuation + No Risk-Off"
...
def archive_existing_latest() -> None:
    ...
    target = ARCHIVE / f"before_nxt34_long_only_pullback_{stamp}"
...
def build_docx(result: dict) -> None:
    ...
    doc.add_heading("NXT v3.4 Latest System And Indicators", 0)
    doc.add_paragraph("Current promoted latest system: NXT v3.4 with SSL14 and LONG-only pullback/touch EMA20 continuation.")
...
def fix_workbook_labels(path: Path) -> None:
    ...
    if "Summary" in wb.sheetnames:
        ws = wb["Summary"]
        ws["A1"] = "NXT v3.4 Latest - LONG-only Pullback Continuation"
        ws["A2"] = "BTC/SOL/SUI 6Y | SSL14 | Runner A | anti-immediate-reversal | LONG-only pullback continuation | no risk-off."
    if "Trades" in wb.sheetnames:
        wb["Trades"]["A1"] = "Detailed Trades - NXT v3.4 LONG-only Pullback Continuation"
    if "Continuation Trades" in wb.sheetnames:
        wb["Continuation Trades"]["A1"] = "Continuation Trades - NXT v3.4 LONG-only Pullback"
    wb.save(path)
...
def main() -> None:
    ...
    try:
        ...
        if not (TEST_JSON.exists() and TEST_XLSX.exists()):
            raise
        print(f"Reusing existing test artifacts because workbook is locked: {exc}")
    ...
    LATEST_SUMMARY.write_text(
        "\n".join(
            [
                ...
                "",
                "Notes: Uses Binance native 1D candles, SSL Channel period 14, Runner A, anti-immediate-reversal, LONG-only pullback/touch EMA20 continuation, and no risk-off.",
                "",
