...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_1y_v21");
const jsonPath = path.join(outDir, "nxt_crypto_btc_sol_sui_1y_v21_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_SOL_SUI_1Y_V21_Backtest.xlsx");
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System v2.1 - BTC SOL SUI 1Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source} | Net of fee/slippage model`);
...
const data = {
  ...
  assumptions: [
    ...
    "Version 2.1 keeps Binance Net Volume and the fee/slippage cost model, but relaxes the Weekly regime filter versus v2.",
    ...
    "Weekly bullish regime requires weekly close > Weekly EMA20 and weekly SSL bullish. Longs are allowed only in bullish weekly regime.",
    "Weekly bearish regime requires weekly close < Weekly EMA20 and weekly SSL bearish. Shorts are allowed only in bearish weekly regime.",
