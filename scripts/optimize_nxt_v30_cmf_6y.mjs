...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_6y_v30_cmf_grid");
const jsonPath = path.join(outDir, "nxt_v30_cmf_grid_6y_results.json");
const xlsxPath = path.join(outDir, "NXT_V30_CMF_Grid_6Y_BTC_SOL_SUI.xlsx");
...
const closeAtr = 2.5;
const cmfPeriod = 20;
const cmfThresholds = [0, 0.05, 0.10];
...
function cmf(candles, period) {
  ...
  for (let i = 0; i < candles.length; i++) {
    ...
    if (i >= period - 1 && volSum !== 0) out[i] = mfvSum / volSum;
  }
  return out;
}
...
function enrich(candles) {
  ...
  const cmf20 = cmf(candles, cmfPeriod);
...
function backtestSymbol(symbol, daily, weekly, cmfThreshold, tp1Atr = closeAtr) {
  const trades = [];
  let position = null;
  let tradeNo = 1;
  ...
  for (let i = 51; i < daily.length - 1; i++) {
      ...
      if (side === "LONG") {
        if (c.low <= position.stop) {
          ...
          if (!position.tp1Done && c.high >= position.tp1) {
            ...
            position.realizedR += 0.5 * ((position.tp1 - position.entryPrice) / position.riskPerUnit);
            position.stop = position.entryPrice;
            position.notes.push(`TP1 hit ${iso(c.time)}; 50% closed and stop moved to breakeven`);
          }
        ...
        if (c.high >= position.stop) {
          ...
          if (!position.tp1Done && c.low <= position.tp1) {
            position.tp1Done = true;
            position.tp1Time = c.time;
            position.realizedR += 0.5 * ((position.entryPrice - position.tp1) / position.riskPerUnit);
            position.stop = position.entryPrice;
            position.notes.push(`TP1 hit ${iso(c.time)}; 50% closed and stop moved to breakeven`);
          }
    ...
    if ([prev.ema20, prev.ema50, prev.atr14, prev.ssl, c.ema20, c.ema50, c.atr14, c.ssl, c.cmf20].some(v => v == null)) continue;
    ...
    const longOk = sslBullCross && priceCrossUpEma20 && distanceOk && c.cmf20 > cmfThreshold;
    const shortOk = sslBearCross && priceCrossDownEma20 && distanceOk && c.cmf20 < -cmfThreshold;
    ...
    position = {
      ...
      notes: [
        ...
        `CMF(${cmfPeriod}) ${c.cmf20.toFixed(3)} with threshold ${cmfThreshold}`,
        `TP1 ${tp1Atr.toFixed(1)} x ATR(14)`,
...
function statsForThreshold(trades, cmfThreshold) {
  return {
    ...overallStats(trades, closeAtr),
    cmfPeriod,
    cmfThreshold,
  };
}
...
async function buildWorkbook(data) {
  ...
  const ranking = workbook.worksheets.add("CMF Grid");
  baseSheet(ranking, "NXT v3.0 CMF(20) Grid - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | Full exit at ${closeAtr} x ATR | CMF replaces Net Volume`);
  const rankingRows = [[
    "Rank", "CMF Period", "Threshold", "Trades", "Wins", "Losses", "Win Rate", "TP Hit Rate", "Total R", "Avg R", "Max DD", "Best R", "Worst R", "BTC R", "SOL R", "SUI R",
  ], ...data.results.map((r, i) => [
    i + 1, r.cmfPeriod, r.cmfThreshold, r.trades, r.wins, r.losses, r.winRate, r.tp1HitRate, r.totalR, r.avgR, r.maxDrawdownR, r.bestR, r.worstR,
  ...
  writeMatrix(ranking, "A4", rankingRows);
  styleHeader(ranking, "A4:P4");
  numberFormat(ranking, `G5:H${rankingRows.length + 3}`, "0.0%");
  numberFormat(ranking, `I5:P${rankingRows.length + 3}`, "0.00");
  ranking.freezePanes.freezeRows(4);
  ranking.getRange(`A4:P${rankingRows.length + 3}`).format.wrapText = true;
  finishSheet(ranking, `A1:P${rankingRows.length + 3}`);
  ...
  baseSheet(summary, "NXT v3.0 Best CMF Filter - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | CMF(${cmfPeriod}) threshold ${data.best.cmfThreshold} | Source: ${data.source} | Net of fee/slippage model`);
  writeMatrix(summary, "A4", [
    ...
    ["Max drawdown (R)", data.best.maxDrawdownR],
  ...
  const tradeHeaders = [
    ...
    "Weekly Regime", "EMA20", "EMA50", "ATR14", "SSL Signal", "Net Volume", "CMF20", "Distance EMA50 ATR", "Notes",
  ];
  const tradeToRow = t => [
    ...
    t.atr14, t.sslAtSignal, t.netVolume, t.cmf20, t.distanceToEma50Atr, t.notes,
  ];
  const trades = workbook.worksheets.add("Trades");
  baseSheet(trades, "Detailed Trades", "One completed trade per row; NXT v2.3 entry logic with full exit at 2.5 ATR.");
  writeMatrix(trades, "A4", [tradeHeaders, ...data.trades.map(tradeToRow)]);
  styleHeader(trades, "A4:AC4");
  ...
  if (data.trades.length) {
    ...
    trades.getRange(`A4:AC${lastTradeRow}`).format.borders = { preset: "inside", style: "thin", color: "#E5E7EB" };
  }
  trades.getRange(`A4:AC${Math.max(lastTradeRow, 5)}`).format.wrapText = true;
  finishSheet(trades, `A1:AC${Math.max(lastTradeRow, 5)}`);
  ...
  baseSheet(assumptions, "Backtest Assumptions", "Objective translation of NXT_Trading_System.docx with full exit at 2.5 ATR.");
  ...
  for (const symbol of symbols) {
    ...
    styleHeader(sheet, "A4:AC4");
    sheet.freezePanes.freezeRows(4);
    const endRow = rows.length + 4;
    if (rows.length) sheet.getRange(`A4:AC${endRow}`).format.borders = { preset: "inside", style: "thin", color: "#E5E7EB" };
    sheet.getRange(`A4:AC${Math.max(endRow, 5)}`).format.wrapText = true;
    finishSheet(sheet, `A1:AC${Math.max(endRow, 5)}`);
  }
...
const runs = cmfThresholds.map(cmfThreshold => {
  const trades = symbols.flatMap(symbol => backtestSymbol(symbol, enriched[symbol], weeklyBySymbol[symbol], cmfThreshold, closeAtr));
  trades.sort((a, b) => new Date(a.exitTime) - new Date(b.exitTime));
  return { cmfThreshold, trades, stats: statsForThreshold(trades, cmfThreshold) };
});
runs.sort((a, b) => {
  if (b.stats.totalR !== a.stats.totalR) return b.stats.totalR - a.stats.totalR;
  if (b.stats.avgR !== a.stats.avgR) return b.stats.avgR - a.stats.avgR;
  return b.stats.maxDrawdownR - a.stats.maxDrawdownR;
});

const bestRun = runs[0];
const trades = bestRun.trades;
...
const data = {
  ...
  assumptions: [
    ...
    "Version 3.0 keeps v2.3 entry structure and fee/slippage cost model, removes the Weekly regime filter, allows EMA20 cross within the last 3 candles, raises the EMA50 distance limit to 2 ATR, and exits fully at 2.5 ATR.",
    ...
    `Long entry requires SSL 10/10 bullish crossover, price crossing above EMA20 within the last 3 candles, distance from close to EMA50 <= 2 ATR(14), and CMF(${cmfPeriod}) > threshold.`,
    `Short entry requires SSL 10/10 bearish crossover, price crossing below EMA20 within the last 3 candles, distance from close to EMA50 <= 2 ATR(14), and CMF(${cmfPeriod}) < -threshold.`,
    ...
    `CMF grid tests thresholds ${cmfThresholds.join(", ")}. CMF uses OHLCV only, so it can run on Binance or Yahoo daily data.`,
    "Stop loss is 1.5 x ATR(14) from entry.",
    `Full position exits at ${closeAtr} x ATR(14). This is implemented as TP1 and TP2 at the same 2.5 ATR level, so both halves close when that level is touched.`,
    "Remaining position can also exit on opposite SSL flip, stop, or end-of-test mark-to-market if 2.5 ATR is not reached.",
