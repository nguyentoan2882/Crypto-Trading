...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_3y_v23_tp1_grid");
const jsonPath = path.join(outDir, "nxt_v23_tp1_grid_3y_results.json");
const xlsxPath = path.join(outDir, "NXT_V23_TP1_Grid_3Y_BTC_SOL_SUI.xlsx");
...
const tp1AtrValues = [1.5, 1.6, 1.7, 1.8, 1.9, 2.0, 2.1, 2.2, 2.3, 2.4, 2.5];
...
function backtestSymbol(symbol, daily, weekly, tp1Atr) {
  const trades = [];
  let position = null;
  let tradeNo = 1;
...
async function buildWorkbook(data) {
  ...
  const ranking = workbook.worksheets.add("TP1 Grid");
  baseSheet(ranking, "NXT v2.3 TP1 Grid - BTC SOL SUI 3Y", `${data.period.start} to ${data.period.end} | Only TP1 ATR is varied`);
  const rankRows = [[
    "Rank", "TP1 ATR", "Trades", "Wins", "Losses", "Win Rate", "TP1 Hit Rate", "Total R", "Average R / Trade", "Max DD", "Best R", "Worst R", "BTC R", "SOL R", "SUI R",
  ], ...data.optimizationResults.map((r, i) => [
    i + 1, r.tp1Atr, r.trades, r.wins, r.losses, r.winRate, r.tp1HitRate, r.totalR, r.avgR, r.maxDrawdownR, r.bestR, r.worstR,
  ...
  numberFormat(ranking, `F5:G${rankRows.length + 3}`, "0.0%");
  ...
  baseSheet(summary, "NXT Trading System v2.3 - Best TP1 Variant", `${data.period.start} to ${data.period.end} | TP1 ${data.best.tp1Atr} x ATR | Source: ${data.source} | Net of fee/slippage model`);
  ...
  baseSheet(trades, "Best TP1 Detailed Trades", "One completed trade per row for the best TP1 setting; all other NXT v2.3 rules unchanged.");
  ...
  baseSheet(assumptions, "TP1 Grid Assumptions", "Objective translation of NXT_Trading_System.docx with TP1 sensitivity test.");
...
const runs = tp1AtrValues.map(tp1Atr => {
  const trades = symbols.flatMap(symbol => backtestSymbol(symbol, enriched[symbol], weeklyBySymbol[symbol], tp1Atr));
  trades.sort((a, b) => new Date(a.exitTime) - new Date(b.exitTime));
  return { tp1Atr, trades, stats: overallStats(trades, tp1Atr) };
});
...
const data = {
  ...
  assumptions: [
    ...
    `TP1 grid tests ${tp1AtrValues[0]} to ${tp1AtrValues[tp1AtrValues.length - 1]} x ATR(14) in 0.1 increments, closing 50% and moving stop to breakeven.`,
