...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_1y_v2");
const jsonPath = path.join(outDir, "nxt_crypto_btc_sol_sui_1y_v2_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_SOL_SUI_1Y_V2_Backtest.xlsx");
...
function weeklyRegimeAt(weekly, time) {
  ...
  if (w.close > w.ema20 && w.close > w.ema50 && w.ssl === 1) return "BULL";
  if (w.close < w.ema20 && w.close < w.ema50 && w.ssl === -1) return "BEAR";
  return "NEUTRAL";
}
...
function backtestSymbol(symbol, daily, weekly) {
  ...
  for (let i = 51; i < daily.length - 1; i++) {
    ...
    const weeklyRegime = weeklyRegimeAt(weekly, c.time);
    const distanceOk = distanceToEma50Atr <= 1.5;
    const longOk = weeklyRegime === "BULL" && sslBullCross && priceCrossUpEma20 && c.netVolume > 0 && distanceOk;
    const shortOk = weeklyRegime === "BEAR" && sslBearCross && priceCrossDownEma20 && c.netVolume < 0 && distanceOk;
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System v2 - BTC SOL SUI 1Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source} | Net of fee/slippage model`);
...
const data = {
  ...
  assumptions: [
    ...
    "Version 2 adds a Weekly regime filter and a fee/slippage cost model to the baseline NXT rules.",
    "Timeframe: Daily entries under Weekly regime. Signals are evaluated on closed daily candles and entered at the next daily open.",
    "Weekly bullish regime requires weekly close > Weekly EMA20, weekly close > Weekly EMA50, and weekly SSL bullish. Longs are allowed only in bullish weekly regime.",
    "Weekly bearish regime requires weekly close < Weekly EMA20, weekly close < Weekly EMA50, and weekly SSL bearish. Shorts are allowed only in bearish weekly regime.",
