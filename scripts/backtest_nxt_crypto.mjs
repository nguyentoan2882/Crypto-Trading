...
const start = Date.UTC(2025, 4, 17, 0, 0, 0);
const warmupStart = Date.UTC(2024, 10, 1, 0, 0, 0);
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_1y");
const jsonPath = path.join(outDir, "nxt_crypto_btc_sol_sui_1y_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_SOL_SUI_1Y_Backtest.xlsx");
const yahooSymbols = {
  BTCUSDT: "BTC-USD",
  SOLUSDT: "SOL-USD",
...
function backtestSymbol(symbol, daily) {
  const trades = [];
  let position = null;
  let tradeNo = 1;
  ...
  for (let i = 51; i < daily.length - 1; i++) {
    ...
    if (position) {
      ...
      if (exitPrice != null) {
        ...
        trades.push({
          ...
          exitPrice,
          exitReason,
          rMultiple: position.realizedR + remainingWeight * remainingR,
    ...
    const priceCrossUpEma20 = crossedUp(c, prev, "ema20");
    const priceCrossDownEma20 = crossedDown(c, prev, "ema20");
    const distanceToEma50Atr = Math.abs(c.close - c.ema50) / c.atr14;
    const distanceOk = distanceToEma50Atr <= 1.5;
    ...
    position = {
      ...
      notes: [
        ...
        `Price crossed ${side === "LONG" ? "above" : "below"} EMA20`,
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System - BTC SOL SUI 1Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source}`);
  ...
  const tradeHeaders = [
    ...
    "TP1", "TP1 Time", "TP2", "Exit Time", "Exit Price", "Exit Reason", "R Multiple", "% Move", "TP1 Hit",
    "EMA20", "EMA50", "ATR14", "SSL Signal", "Net Volume", "Distance EMA50 ATR", "Notes",
  ];
  const tradeToRow = t => [
    ...
    new Date(t.exitTime), t.exitPrice, t.exitReason, t.rMultiple, t.pctMove, t.tp1Hit, t.ema20, t.ema50,
    t.atr14, t.sslAtSignal, t.netVolume, t.distanceToEma50Atr, t.notes,
  ];
  const trades = workbook.worksheets.add("Trades");
  baseSheet(trades, "Detailed Trades", "One completed trade per row; Daily signal, next daily open entry, NXT exits.");
  writeMatrix(trades, "A4", [tradeHeaders, ...data.trades.map(tradeToRow)]);
  styleHeader(trades, "A4:Y4");
  ...
  if (data.trades.length) {
    ...
    numberFormat(trades, `P5:P${lastTradeRow}`, "0.00");
    numberFormat(trades, `Q5:Q${lastTradeRow}`, "0.00%");
    numberFormat(trades, `S5:U${lastTradeRow}`, "0.000000");
    numberFormat(trades, `W5:X${lastTradeRow}`, "0.00");
    trades.getRange(`A4:Y${lastTradeRow}`).format.borders = { preset: "inside", style: "thin", color: "#E5E7EB" };
  }
  trades.getRange(`A4:Y${Math.max(lastTradeRow, 5)}`).format.wrapText = true;
  finishSheet(trades, `A1:Y${Math.max(lastTradeRow, 5)}`);
  ...
  baseSheet(assumptions, "Backtest Assumptions", "Objective translation of NXT_Trading_System.docx.");
  ...
  for (const symbol of symbols) {
    ...
    styleHeader(sheet, "A4:Y4");
    sheet.freezePanes.freezeRows(4);
    const endRow = rows.length + 4;
    if (rows.length) sheet.getRange(`A4:Y${endRow}`).format.borders = { preset: "inside", style: "thin", color: "#E5E7EB" };
    sheet.getRange(`A4:Y${Math.max(endRow, 5)}`).format.wrapText = true;
    finishSheet(sheet, `A1:Y${Math.max(endRow, 5)}`);
  }
...
const trades = symbols.flatMap(symbol => backtestSymbol(symbol, enriched[symbol]));
...
const data = {
  ...
  assumptions: [
    ...
    "Timeframe: Daily. Signals are evaluated on closed daily candles and entered at the next daily open.",
    "Long entry requires SSL 10/10 bullish crossover, price crossing above EMA20, Net Volume > 0, and distance from close to EMA50 <= 1.5 ATR(14).",
    "Short entry requires SSL 10/10 bearish crossover, price crossing below EMA20, Net Volume < 0, and distance from close to EMA50 <= 1.5 ATR(14).",
    ...
    "Net Volume is approximated from daily candle direction: positive volume for bullish candles, negative volume for bearish candles, zero for unchanged candles.",
    "Stop loss is 1.5 x ATR(14) from entry.",
    "TP1 is 1.5 x ATR(14), closing 50% and moving stop to breakeven.",
    "TP2 is 2.5 x ATR(14), closing the remaining 50%. Remaining position can also exit on opposite SSL flip, stop, or end-of-test mark-to-market.",
    ...
    "Results are expressed in R multiples before exchange fees, slippage, funding, borrowing cost, and taxes.",
  ],
};
