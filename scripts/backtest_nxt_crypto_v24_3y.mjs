...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_3y_v24");
const jsonPath = path.join(outDir, "nxt_crypto_btc_sol_sui_3y_v24_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_SOL_SUI_3Y_V24_Backtest.xlsx");
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System v2.4 - BTC SOL SUI 3Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source} | Net of fee/slippage model`);
...
const data = {
  ...
  assumptions: [
    ...
    "Version 2.4 keeps v2.3 entries, replaces fixed TP2 with a runner, and trails the remaining 50% after TP1 using a 1.5 ATR stop until SSL exit or trail stop.",
