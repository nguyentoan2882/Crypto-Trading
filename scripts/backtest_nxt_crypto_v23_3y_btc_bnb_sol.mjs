...
const outDir = path.resolve("outputs", "nxt_crypto_btc_bnb_sol_3y_v23");
const jsonPath = path.join(outDir, "nxt_crypto_btc_bnb_sol_3y_v23_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_BNB_SOL_3Y_V23_Backtest.xlsx");
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System v2.3 - BTC BNB SOL 3Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source} | Net of fee/slippage model`);
