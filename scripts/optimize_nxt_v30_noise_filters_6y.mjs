...
const outDir = path.resolve("outputs", "nxt_v30_noise_filter_grid_6y");
const jsonPath = path.join(outDir, "nxt_v30_noise_filter_grid_6y_results.json");
const xlsxPath = path.join(outDir, "NXT_V30_Noise_Filter_Grid_6Y_BTC_SOL_SUI.xlsx");
...
const filterVariants = [
  {
    key: "baseline_net_volume",
    name: "Baseline Net Volume",
    description: "LONG Net Volume > 0; SHORT Net Volume < 0",
    pass: (side, c) => side === "LONG" ? c.netVolume > 0 : c.netVolume < 0,
  },
  {
    key: "macd_zero",
    name: "MACD zero-line",
    description: "LONG MACD line > 0; SHORT MACD line < 0",
    pass: (side, c) => c.macdLine != null && (side === "LONG" ? c.macdLine > 0 : c.macdLine < 0),
  },
  {
    key: "rsi_50",
    name: "RSI regime 50",
  ...
  },
  {
    key: "adx_di_20",
    name: "ADX/DI trend",
    description: "ADX14 > 20 and DI direction agrees with trade direction",
    pass: (side, c) => c.adx14 != null && c.plusDi14 != null && c.minusDi14 != null && c.adx14 > 20 && (side === "LONG" ? c.plusDi14 > c.minusDi14 : c.minusDi14 > c.plusDi14),
  },
  {
    key: "ema50_slope5",
    name: "EMA50 slope 5D",
    description: "LONG EMA50 slope over 5 days > 0; SHORT < 0",
    pass: (side, c) => c.ema50Slope5 != null && (side === "LONG" ? c.ema50Slope5 > 0 : c.ema50Slope5 < 0),
  },
  {
    key: "obv_slope10",
    name: "OBV slope 10D",
    description: "LONG OBV slope over 10 days > 0; SHORT < 0",
    pass: (side, c) => c.obvSlope10 != null && (side === "LONG" ? c.obvSlope10 > 0 : c.obvSlope10 < 0),
  },
];

function backtestSymbol(symbol, daily, weekly, variant, tp1Atr = closeAtr) {
  const trades = [];
  let position = null;
  let tradeNo = 1;
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT v3.0 Correct Full Close 2.5 ATR - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | Full exit ${closeAtr} ATR | Source: ${data.source}`);
...
const variants = filterVariants.map(variant => {
  const trades = symbols.flatMap(symbol => backtestSymbol(symbol, enriched[symbol], weeklyBySymbol[symbol], variant, closeAtr));
  trades.sort((a, b) => new Date(a.exitTime) - new Date(b.exitTime));
  return {
    key: variant.key,
    name: variant.name,
    description: variant.description,
    stats: overallStats(trades, closeAtr),
    summary: summarize(trades),
    trades,
  };
});
const ranking = variants
  .map(v => ({ key: v.key, name: v.name, description: v.description, ...v.stats }))
...
const data = {
  ...
  assumptions: [
    ...
    "Noise-filter grid keeps the corrected NXT v3 entry/exit structure and swaps the final directional/noise filter.",
    ...
    "Tested filters: Net Volume baseline, MACD zero-line, RSI14 regime, ADX/DI trend strength, EMA50 5-day slope, and OBV 10-day slope.",
