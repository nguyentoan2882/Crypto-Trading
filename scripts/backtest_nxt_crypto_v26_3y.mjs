...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_3y_v26");
const jsonPath = path.join(outDir, "nxt_crypto_btc_sol_sui_3y_v26_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_SOL_SUI_3Y_V26_Backtest.xlsx");
...
function backtestSymbol(symbol, daily, weekly) {
  ...
  for (let i = 51; i < daily.length - 1; i++) {
      ...
      if (side === "LONG") {
        if (c.low <= position.stop) {
          ...
          if (!position.tp1Done && c.high >= position.tp1) {
            ...
            position.notes.push(`TP1 hit ${iso(c.time)}; 50% closed and runner stop moved to breakeven`);
          }
    ...
    position = {
      ...
      runnerReference: side === "LONG" ? entryPrice + c.atr14 * 2.5 : entryPrice - c.atr14 * 2.5,
      tp1Done: false,
      tp1Time: null,
      realizedR: 0,
      ema20: c.ema20,
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System v2.6 - BTC SOL SUI 3Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source} | Net of fee/slippage model`);
...
const data = {
  ...
  assumptions: [
    ...
    "Version 2.6 keeps v2.3 entries, closes 50% at TP1 = 1.5 ATR, moves the runner stop to breakeven after TP1, and starts 1.5 ATR trailing from the following daily candle onward.",
    ...
    "TP1 is 1.5 x ATR(14), closing 50% and moving runner stop to breakeven.",
    "The remaining 50% is no longer closed at fixed TP2. From the daily candle after TP1 onward, it trails using 1.5 x ATR(14) from each close and exits on opposite SSL flip, trail stop, or end-of-test mark-to-market.",
