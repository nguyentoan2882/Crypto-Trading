...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_3y_v22");
const jsonPath = path.join(outDir, "nxt_crypto_btc_sol_sui_3y_v22_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_SOL_SUI_3Y_V22_Backtest.xlsx");
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System v2.2 - BTC SOL SUI 3Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source} | Net of fee/slippage model`);
