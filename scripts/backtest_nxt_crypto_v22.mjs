...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_1y_v22");
const jsonPath = path.join(outDir, "nxt_crypto_btc_sol_sui_1y_v22_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_SOL_SUI_1Y_V22_Backtest.xlsx");
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System v2.2 - BTC SOL SUI 1Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source} | Net of fee/slippage model`);
...
const data = {
  ...
  assumptions: [
    ...
    "Version 2.2 keeps Binance Net Volume and the fee/slippage cost model, but removes the Weekly regime filter.",
