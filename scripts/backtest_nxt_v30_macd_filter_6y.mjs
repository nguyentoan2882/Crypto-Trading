...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_6y_v30_macd_filter");
const jsonPath = path.join(outDir, "nxt_v30_macd_filter_6y_results.json");
const xlsxPath = path.join(outDir, "NXT_V30_MACD_Filter_6Y_BTC_SOL_SUI_20K.xlsx");
...
function enrich(candles) {
  ...
  return candles.map((c, i) => ({
    ...
    ema50: ema50[i],
    macdLine: macdLine[i],
    macdSignal: macdLine[i] == null ? null : macdSignal[i],
    macdHistogram: macdLine[i] == null || macdSignal[i] == null ? null : macdLine[i] - macdSignal[i],
    atr14: atr14[i],
    ssl: ssl[i],
...
function backtestSymbol(symbol, daily, weekly, tp1Atr = closeAtr) {
  ...
  for (let i = 51; i < daily.length - 1; i++) {
    ...
    if (position) {
      ...
      if (exitPrice != null) {
        ...
        trades.push({
          ...
          macdLine: position.macdLine,
          macdSignal: position.macdSignal,
          macdHistogram: position.macdHistogram,
    ...
    if ([prev.ema20, prev.ema50, prev.atr14, prev.ssl, c.ema20, c.ema50, c.atr14, c.ssl, c.macdLine].some(v => v == null)) continue;
    ...
    const longOk = sslBullCross && priceCrossUpEma20 && c.macdLine > 0 && distanceOk;
    const shortOk = sslBearCross && priceCrossDownEma20 && c.macdLine < 0 && distanceOk;
    ...
    position = {
      ...
      notes: [
        ...
        `MACD line ${c.macdLine.toFixed(2)} ${side === "LONG" ? ">" : "<"} 0`,
...
function clearSheet(sheet, rows, cols) {
...
async function buildWorkbook(data) {
  ...
  clearSheet(summary, 40, 16);
  baseSheet(summary, "NXT v3.0 MACD Filter - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | MACD line zero filter | Template: NXT_Backtest_Workbook_Template.xlsx`);
  ...
  const tradeHeaders = [
    ...
    "Weekly Regime", "EMA20", "EMA50", "ATR14", "SSL Signal", "MACD Line", "MACD Signal", "MACD Histogram", "Distance EMA50 ATR", "Notes",
  ];
  const tradeToRow = t => [
    ...
    t.atr14, t.sslAtSignal, t.macdLine, t.macdSignal, t.macdHistogram, t.distanceToEma50Atr, t.notes,
  ];
  const trades = getSheet(workbook, "Trades");
  clearSheet(trades, Math.max(data.trades.length + 10, 220), 28);
  ...
  const curve = getSheet(workbook, "Equity Curve");
  clearSheet(curve, Math.max(data.trades.length + 10, 220), 8);
  ...
  clearSheet(assumptions, 40, 2);
  baseSheet(assumptions, "Backtest Assumptions", "NXT v3.0 with MACD replacing Net Volume.");
  ...
  for (const symbol of symbols) {
    const sheet = getSheet(workbook, symbol.replace("USDT", ""));
    clearSheet(sheet, Math.max(data.bySymbol[symbol].length + 10, 100), 28);
...
const data = {
  ...
  assumptions: [
    ...
    "This MACD-filter variant keeps the corrected NXT v3 entry/exit structure and replaces the Net Volume directional filter with MACD zero-line direction.",
    ...
    "Long entry requires SSL 10/10 bullish crossover, price crossing above EMA20 within the last 3 candles, MACD line > 0, and distance from close to EMA50 <= 2 ATR(14).",
    "Short entry requires SSL 10/10 bearish crossover, price crossing below EMA20 within the last 3 candles, MACD line < 0, and distance from close to EMA50 <= 2 ATR(14).",
    ...
    "MACD line is EMA(12) - EMA(26); the zero-line condition uses the MACD line, not the histogram.",
