...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_3y_v30");
const jsonPath = path.join(outDir, "nxt_crypto_btc_sol_sui_3y_v30_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_SOL_SUI_3Y_V30_Backtest.xlsx");
...
function backtestSymbol(symbol, daily, weekly) {
  ...
  for (let i = 51; i < daily.length - 1; i++) {
    ...
    if (position) {
      ...
      let remainingWeight = 1 - position.realizedWeight;
      ...
      if (side === "LONG") {
        if (c.low <= position.stop) {
          ...
          exitReason = position.tp1Done ? "ATR trailing stop after TP1" : "Stop loss";
        } else {
          if (!position.tp1Done && c.high >= position.tp1) {
            position.tp1Done = true;
            position.tp1Time = c.time;
            position.realizedR += 0.4 * ((position.tp1 - position.entryPrice) / position.riskPerUnit);
            position.realizedWeight += 0.4;
            remainingWeight = 1 - position.realizedWeight;
            position.stop = position.entryPrice;
            position.notes.push(`TP1 hit ${iso(c.time)}; 40% closed and remaining stop moved to breakeven`);
          }
          if (position.tp1Done && !position.tp2Done && c.high >= position.tp2) {
            position.tp2Done = true;
            position.tp2Time = c.time;
            position.realizedR += 0.3 * ((position.tp2 - position.entryPrice) / position.riskPerUnit);
            position.realizedWeight += 0.3;
            remainingWeight = 1 - position.realizedWeight;
            position.notes.push(`TP2 hit ${iso(c.time)}; additional 30% closed; 30% runner remains`);
          }
          if (position.tp1Done && c.time > position.tp1Time) {
            const trailedStop = c.close - c.atr14 * 1.5;
            if (trailedStop > position.stop) {
              position.stop = trailedStop;
              position.notes.push(`Runner trail raised ${iso(c.time)} @ ${position.stop.toFixed(6)}`);
            }
          }
          if (sslFlip) {
            exitPrice = c.close;
        ...
        if (c.high >= position.stop) {
          ...
          if (!position.tp1Done && c.low <= position.tp1) {
            position.tp1Done = true;
            position.tp1Time = c.time;
            position.realizedR += 0.4 * ((position.entryPrice - position.tp1) / position.riskPerUnit);
            position.realizedWeight += 0.4;
            remainingWeight = 1 - position.realizedWeight;
            position.stop = position.entryPrice;
            position.notes.push(`TP1 hit ${iso(c.time)}; 40% closed and remaining stop moved to breakeven`);
          ...
          if (position.tp1Done && !position.tp2Done && c.low <= position.tp2) {
            position.tp2Done = true;
            position.tp2Time = c.time;
            position.realizedR += 0.3 * ((position.entryPrice - position.tp2) / position.riskPerUnit);
            position.realizedWeight += 0.3;
            remainingWeight = 1 - position.realizedWeight;
            position.notes.push(`TP2 hit ${iso(c.time)}; additional 30% closed; 30% runner remains`);
          }
          if (position.tp1Done && c.time > position.tp1Time) {
            ...
            if (trailedStop < position.stop) {
              position.stop = trailedStop;
              position.notes.push(`Runner trail lowered ${iso(c.time)} @ ${position.stop.toFixed(6)}`);
            }
          }
          if (sslFlip) {
            exitPrice = c.close;
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System v3.0 - BTC SOL SUI 3Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source} | Net of fee/slippage model`);
  ...
  const tradeToRow = t => [
    ...
    t.initialStop, t.finalStop, t.riskPerUnit, t.tp1, t.tp1Time ? new Date(t.tp1Time) : "", t.runnerReference,
...
const data = {
  ...
  assumptions: [
    ...
    "Version 3.0 keeps v2.3 entries, closes 40% at TP1 = 1.5 ATR, closes 30% at TP2 = 2.5 ATR, and runs the remaining 30% with the v2.6 breakeven plus next-day ATR trailing model.",
    ...
    "TP1 is 1.5 x ATR(14), closing 40% and moving the remaining stop to breakeven.",
    "TP2 is 2.5 x ATR(14), closing an additional 30% if reached. The remaining 30% trails using 1.5 x ATR(14) from each close after TP1 and exits on opposite SSL flip, trail stop, or end-of-test mark-to-market.",
    "Conservative intraday ordering: stop is checked before TP1 when multiple levels could be touched inside one daily candle. Runner trail does not update on the same candle that hits TP1.",
