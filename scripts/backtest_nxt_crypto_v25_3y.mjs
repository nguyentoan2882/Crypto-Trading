...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_3y_v25");
const jsonPath = path.join(outDir, "nxt_crypto_btc_sol_sui_3y_v25_results.json");
const xlsxPath = path.join(outDir, "NXT_Crypto_BTC_SOL_SUI_3Y_V25_Backtest.xlsx");
    ...
    if (position) {
      ...
      if (side === "LONG") {
        if (c.low <= position.stop) {
          ...
          exitReason = position.tp1Done
            ? Math.abs(position.stop - position.tp1) < 1e-9 ? "Runner stop at TP1 after TP1" : "ATR trailing stop after TP1"
            : "Stop loss";
        } else {
          if (!position.tp1Done && c.high >= position.tp1) {
            ...
            position.notes.push(`TP1 hit ${iso(c.time)}; 50% closed and runner stop moved to TP1`);
          }
          if (position.tp1Done) {
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT Trading System v2.5 - BTC SOL SUI 3Y Backtest", `${data.period.start} to ${data.period.end} | Source: ${data.source} | Net of fee/slippage model`);
...
const data = {
  ...
  assumptions: [
    ...
    "Version 2.5 keeps v2.3 entries, replaces fixed TP2 with a runner, moves the remaining 50% stop to TP1 after TP1 is hit, and then trails using a 1.5 ATR stop until SSL exit or trail stop.",
    ...
    "TP1 is 1.5 x ATR(14), closing 50% and moving runner stop to TP1.",
    "The remaining 50% is no longer closed at fixed TP2. It trails using 1.5 x ATR(14) from each closed candle and exits on opposite SSL flip, trail stop, or end-of-test mark-to-market.",
    "Conservative intraday ordering: stop is checked before TP1 when multiple levels could be touched inside one daily candle. Runner trail updates after the signal candle closes.",
