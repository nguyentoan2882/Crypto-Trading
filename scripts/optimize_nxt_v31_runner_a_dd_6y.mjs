...
const outDir = path.resolve("outputs", "nxt_v31_runner_a_dd_grid_6y");
const jsonPath = path.join(outDir, "nxt_v31_runner_a_dd_grid_6y_results.json");
const xlsxPath = path.join(outDir, "NXT_V31_Runner_A_DD_Filter_Grid_6Y_BTC_SOL_SUI_20K.xlsx");
...
const triggerAtr = 2.5;
const breakEvenAtr = triggerAtr;
...
function adx(candles, period = 14) {
  ...
  for (let i = period; i < candles.length; i++) {
    if (!atrSm[i]) continue;
    plusDi[i] = 100 * plusSm[i] / atrSm[i];
    minusDi[i] = 100 * minusSm[i] / atrSm[i];
...
async function fetchBinanceKlines(symbol, interval, from, to) {
  const cachePath = path.resolve("data_cache", "binance_spot_1d", `${symbol}.json`);
  if (interval === "1d") {
    try {
      const cached = JSON.parse(await fs.readFile(cachePath, "utf8"));
      const rows = cached.filter(c => c.time >= from && c.time < to);
...
function enrich(candles) {
  ...
  return candles.map((c, i) => ({
    ...
    ema20: ema20[i],
    ema50: ema50[i],
    ema50Slope5: i >= 5 && ema50[i] != null && ema50[i - 5] != null ? ema50[i] - ema50[i - 5] : null,
    macdLine: macdLine[i],
    rsi14: rsi14[i],
    adx14: adx14.adx[i],
    plusDi14: adx14.plusDi[i],
    minusDi14: adx14.minusDi[i],
    obvSlope10: i >= 10 ? obvRaw[i] - obvRaw[i - 10] : null,
    atr14: atr14[i],
    ssl: ssl[i],
...
const runnerA = { key: "runner_a_50_50_ssl", name: "Runner A 50/50 SSL", lockWeight: 0.5, runnerWeight: 0.5, description: "Close 50% at 2.5 ATR; keep 50% runner until SSL opposite flip or breakeven stop." };

const filterVariants = [
  {
    key: "rsi_50_dist_2",
    name: "RSI 50/50, distance <=2 ATR",
    description: "Current v3.1 filter: LONG RSI14 > 50; SHORT RSI14 < 50; distance to EMA50 <= 2 ATR.",
    maxDistance: 2,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 50 : c.rsi14 < 50),
  },
  {
    key: "rsi_52_48_dist_2",
    name: "RSI 52/48, distance <=2 ATR",
    description: "LONG RSI14 > 52; SHORT RSI14 < 48.",
    maxDistance: 2,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 52 : c.rsi14 < 48),
  },
  {
    key: "rsi_55_45_dist_2",
    name: "RSI 55/45, distance <=2 ATR",
    description: "LONG RSI14 > 55; SHORT RSI14 < 45.",
    maxDistance: 2,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 55 : c.rsi14 < 45),
  },
  {
    key: "rsi_52_48_dist_15",
    name: "RSI 52/48, distance <=1.5 ATR",
    description: "RSI 52/48 with tighter EMA50 distance.",
    maxDistance: 1.5,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 52 : c.rsi14 < 48),
  },
  {
    key: "rsi_52_48_dist_16",
    name: "RSI 52/48, distance <=1.6 ATR",
    description: "RSI 52/48 with EMA50 distance <= 1.6 ATR.",
    maxDistance: 1.6,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 52 : c.rsi14 < 48),
  },
  {
    key: "rsi_52_48_dist_17",
    name: "RSI 52/48, distance <=1.7 ATR",
    description: "RSI 52/48 with EMA50 distance <= 1.7 ATR.",
    maxDistance: 1.7,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 52 : c.rsi14 < 48),
  },
  {
    key: "rsi_53_47_dist_15",
    name: "RSI 53/47, distance <=1.5 ATR",
    description: "LONG RSI14 > 53; SHORT RSI14 < 47; distance <= 1.5 ATR.",
    maxDistance: 1.5,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 53 : c.rsi14 < 47),
  },
  {
    key: "rsi_53_47_dist_16",
    name: "RSI 53/47, distance <=1.6 ATR",
    description: "LONG RSI14 > 53; SHORT RSI14 < 47; distance <= 1.6 ATR.",
    maxDistance: 1.6,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 53 : c.rsi14 < 47),
  },
  {
    key: "rsi_54_46_dist_15",
    name: "RSI 54/46, distance <=1.5 ATR",
    description: "LONG RSI14 > 54; SHORT RSI14 < 46; distance <= 1.5 ATR.",
    maxDistance: 1.5,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 54 : c.rsi14 < 46),
  },
  {
    key: "rsi_55_45_dist_15",
    name: "RSI 55/45, distance <=1.5 ATR",
    description: "RSI 55/45 with tighter EMA50 distance.",
    maxDistance: 1.5,
    pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 55 : c.rsi14 < 45),
  },
  {
    key: "rsi_52_48_adx20",
    name: "RSI 52/48 + ADX/DI",
    description: "RSI 52/48 plus ADX14 > 20 and DI agrees.",
    maxDistance: 2,
    pass: (side, c) => c.rsi14 != null && c.adx14 != null && c.plusDi14 != null && c.minusDi14 != null
      && (side === "LONG" ? c.rsi14 > 52 && c.plusDi14 > c.minusDi14 : c.rsi14 < 48 && c.minusDi14 > c.plusDi14)
      && c.adx14 > 20,
  },
  {
    key: "rsi_52_48_slope5",
    name: "RSI 52/48 + EMA50 slope",
    description: "RSI 52/48 plus EMA50 5-day slope agrees.",
    maxDistance: 2,
    pass: (side, c) => c.rsi14 != null && c.ema50Slope5 != null
      && (side === "LONG" ? c.rsi14 > 52 && c.ema50Slope5 > 0 : c.rsi14 < 48 && c.ema50Slope5 < 0),
  },
  {
    key: "rsi_55_45_slope5",
    name: "RSI 55/45 + EMA50 slope",
    description: "RSI 55/45 plus EMA50 5-day slope agrees.",
    maxDistance: 2,
    pass: (side, c) => c.rsi14 != null && c.ema50Slope5 != null
      && (side === "LONG" ? c.rsi14 > 55 && c.ema50Slope5 > 0 : c.rsi14 < 45 && c.ema50Slope5 < 0),
  },
];

function backtestSymbol(symbol, daily, weekly, variant, runner) {
  const trades = [];
  let position = null;
  let tradeNo = 1;
  ...
  for (let i = 51; i < daily.length - 1; i++) {
    ...
    if (position) {
      ...
      const remainingWeight = position.tp1Done ? position.runnerWeight : 1;
      ...
      if (side === "LONG") {
        if (c.low <= position.stop) {
          ...
          if (!position.tp1Done && c.high >= position.tp2) {
            position.tp1Done = true;
            position.tp1Time = position.tp1Time ?? c.time;
            position.realizedR += position.lockWeight * ((position.tp2 - position.entryPrice) / position.riskPerUnit);
            position.stop = position.entryPrice;
            position.notes.push(`Runner trigger hit ${iso(c.time)}: closed ${(position.lockWeight * 100).toFixed(0)}% at ${triggerAtr} ATR; ${(position.runnerWeight * 100).toFixed(0)}% runner remains`);
          }
          if (sslFlip) {
            exitPrice = c.close;
            exitReason = position.tp1Done ? "Runner exit: SSL bearish flip" : "SSL bearish flip";
          }
        }
      } else {
        if (c.high >= position.stop) {
          ...
          if (!position.tp1Done && c.low <= position.tp2) {
            position.tp1Done = true;
            position.tp1Time = position.tp1Time ?? c.time;
            position.realizedR += position.lockWeight * ((position.entryPrice - position.tp2) / position.riskPerUnit);
            position.stop = position.entryPrice;
            position.notes.push(`Runner trigger hit ${iso(c.time)}: closed ${(position.lockWeight * 100).toFixed(0)}% at ${triggerAtr} ATR; ${(position.runnerWeight * 100).toFixed(0)}% runner remains`);
          }
          if (sslFlip) {
            exitPrice = c.close;
            exitReason = position.tp1Done ? "Runner exit: SSL bullish flip" : "SSL bullish flip";
          }
        }
      }
    ...
    const distanceOk = distanceToEma50Atr <= (variant.maxDistance ?? 2);
    const longOk = sslBullCross && priceCrossUpEma20 && distanceOk && variant.pass("LONG", c);
    const shortOk = sslBearCross && priceCrossDownEma20 && distanceOk && variant.pass("SHORT", c);
    ...
    position = {
      ...
      stop: initialStop,
      riskPerUnit,
      tp1: side === "LONG" ? entryPrice + c.atr14 * triggerAtr : entryPrice - c.atr14 * triggerAtr,
      breakEvenLevel: side === "LONG" ? entryPrice + c.atr14 * breakEvenAtr : entryPrice - c.atr14 * breakEvenAtr,
      tp2: side === "LONG" ? entryPrice + c.atr14 * triggerAtr : entryPrice - c.atr14 * triggerAtr,
      lockWeight: runner.lockWeight,
      runnerWeight: runner.runnerWeight,
      tp1Done: false,
      breakEvenDone: false,
      tp1Time: null,
      realizedR: 0,
      ...
      notes: [
        ...
        `Noise filter: ${variant.name}`,
        `Runner variant: ${runner.name}`,
        `Trigger ${triggerAtr.toFixed(1)} x ATR(14); close ${(runner.lockWeight * 100).toFixed(0)}%, runner ${(runner.runnerWeight * 100).toFixed(0)}% trails SSL flip`,
...
function accountRows(trades) {
  ...
  for (let i = 0; i < trades.length; i++) {
    ...
    equity += pnl;
    peak = Math.max(peak, equity);
    rows.push([i + 1, new Date(t.exitTime), t.symbol.replace("USDT", ""), t.side, t.rMultiple, before, risk, pnl, equity, equity / peak - 1]);
  }
  return rows;
}
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT v3.1 Runner A DD Filter Grid - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | Template: NXT_Backtest_Workbook_Template.xlsx`);
  writeMatrix(summary, "A4", [
    ["Metric", "Value"],
    ["Best variant", data.bestVariant.name],
    ...
    ["Final equity 20K", data.account.finalEquity],
    ["Compound return", data.account.compoundReturn],
  ]);
  styleHeader(summary, "A4:B4");
  numberFormat(summary, "B7:B7", "0.0%");
  numberFormat(summary, "B8:B10", "0.00");
  numberFormat(summary, "B11:B11", "$#,##0");
  ...
  writeMatrix(summary, "A15", [["Variant", "Trades", "Win Rate", "Total R", "Avg R", "Max DD R", "Best R", "Worst R", "Final Equity 20K", "Return"], ...data.ranking.map(r => [
    r.name, r.trades, r.winRate, r.totalR, r.avgR, r.maxDrawdownR, r.bestR, r.worstR, r.account.finalEquity, r.account.compoundReturn,
  ])]);
  styleHeader(summary, "A15:J15");
  numberFormat(summary, "C16:C20", "0.0%");
  numberFormat(summary, "D16:H20", "0.00");
  numberFormat(summary, "I16:I20", "$#,##0");
  numberFormat(summary, "J16:J20", "0.0%");
  ...
  finishSheet(summary, "A1:L20");

  const tradeHeaders = [
    "Symbol", "No", "Side", "Signal Time", "Entry Time", "Entry Price", "Initial Stop", "Final Stop", "Risk / Unit",
    "TP 2.5 ATR", "Trigger Time", "TP Ref", "Exit Time", "Exit Price", "Exit Reason", "Gross R", "Cost R", "Net R", "% Move", "Trigger Hit",
  ...
  baseSheet(trades, `Detailed Trades - ${data.bestVariant.name}`, "One completed trade per row; runner exits on SSL opposite flip after 2.5 ATR trigger.");
  ...
  baseSheet(assumptions, "Backtest Assumptions", "NXT v3.1 Runner A filter grid for drawdown reduction.");
  ...
  const account = workbook.worksheets.add("20K Account");
  baseSheet(account, `20K Account - ${data.bestVariant.name}`, "Compounded sequence at 2.0% risk per trade; leverage is not modeled.");
  const acctRows = accountRows(data.trades);
  writeMatrix(account, "A4", [["Trade", "Exit Time", "Symbol", "Side", "Net R", "Equity Before", "Risk USD", "P/L USD", "Equity After", "Drawdown"], ...acctRows]);
  styleHeader(account, "A4:J4");
  const acctEnd = acctRows.length + 4;
  if (acctRows.length) {
    numberFormat(account, `B5:B${acctEnd}`, "yyyy-mm-dd hh:mm");
    numberFormat(account, `E5:E${acctEnd}`, "0.00");
    numberFormat(account, `F5:I${acctEnd}`, "$#,##0");
    numberFormat(account, `J5:J${acctEnd}`, "0.0%");
  }
  finishSheet(account, `A1:J${Math.max(acctEnd, 5)}`);
  ...
  await execFileAsync(
    ...
    ["scripts\\apply_backtest_template_format.py", xlsxPath],
    { cwd: process.cwd() },
  );
}
...
const variants = filterVariants.map(variant => {
  const trades = symbols.flatMap(symbol => backtestSymbol(symbol, enriched[symbol], weeklyBySymbol[symbol], variant, runnerA));
  trades.sort((a, b) => new Date(a.exitTime) - new Date(b.exitTime));
  return {
    key: variant.key,
    name: variant.name,
    description: variant.description,
    lockWeight: runnerA.lockWeight,
    runnerWeight: runnerA.runnerWeight,
    account: accountStats(trades),
    stats: overallStats(trades, triggerAtr),
    summary: summarize(trades),
    trades,
  };
});
const ranking = variants
  .map(v => ({ key: v.key, name: v.name, description: v.description, lockWeight: v.lockWeight, runnerWeight: v.runnerWeight, account: v.account, ...v.stats }))
  .sort((a, b) => b.totalR - a.totalR);
const bestVariant = variants.find(v => v.key === ranking[0].key);
const trades = bestVariant.trades;
const bySymbol = Object.fromEntries(symbols.map(symbol => [symbol, trades.filter(t => t.symbol === symbol)]));
const stats = bestVariant.stats;
const data = {
  ...
  ranking,
  variants,
  bestVariantKey: bestVariant.key,
  bestVariant: { key: bestVariant.key, name: bestVariant.name, description: bestVariant.description },
  stats,
  account: bestVariant.account,
  summary: bestVariant.summary,
  trades,
  bySymbol,
  datasets,
  assumptions: [
    "Source system: NXT v3.1 RSI 50/50.",
    "Exit logic is fixed to Runner A: close 50% at 2.5 ATR and keep 50% runner until opposite SSL flip or breakeven stop.",
    "This grid only changes entry filters to reduce drawdown.",
    ...
    "Common long entry requires SSL 10/10 bullish crossover, price crossing above EMA20 within the last 3 candles, distance from close to EMA50 <= 2 ATR(14), plus the tested filter condition.",
    "Common short entry requires SSL 10/10 bearish crossover, price crossing below EMA20 within the last 3 candles, distance from close to EMA50 <= 2 ATR(14), plus the tested filter condition.",
    ...
    "Tested filters include RSI thresholds, tighter EMA50 distance, ADX/DI trend confirmation, and EMA50 slope confirmation.",
    "Stop loss is 1.5 x ATR(14) from entry.",
    `Runner trigger is ${triggerAtr} x ATR(14). Runner A closes 50% and keeps 50%; Runner B closes 70% and keeps 30%.`,
    "After trigger, stop moves to breakeven and the remaining runner exits on opposite SSL flip, breakeven stop, or end-of-test mark-to-market.",
...
console.log(JSON.stringify({
  jsonPath,
  xlsxPath,
  bestVariant: { key: bestVariant.key, name: bestVariant.name, stats: bestVariant.stats, summary: bestVariant.summary },
  ranking,
