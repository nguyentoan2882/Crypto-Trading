...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_6y_v23_close_25");
const jsonPath = path.join(outDir, "nxt_v23_close_25_6y_results.json");
const xlsxPath = path.join(outDir, "NXT_V23_Close_25_ATR_6Y_BTC_SOL_SUI.xlsx");
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT v2.3 Close at 2.5 ATR - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | Full exit at ${closeAtr} x ATR | Source: ${data.source} | Net of fee/slippage model`);
