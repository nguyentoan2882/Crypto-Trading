...
const inputJson = "outputs/nxt_v30_noise_filter_grid_6y/nxt_v30_noise_filter_grid_6y_results.json";
const outputXlsx = "outputs/nxt_v30_noise_filter_grid_6y/NXT_V30_Noise_Filter_Grid_6Y_BTC_SOL_SUI.xlsx";
...
clear(summary, 40, 18);
baseSheet(summary, "NXT v3.0 Noise Filter Grid - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | Best: ${best.name} | Template: NXT_Backtest_Workbook_Template.xlsx`);
...
const grid = getSheet(workbook, "Filter Grid");
clear(grid, 40, 14);
baseSheet(grid, "Filter Grid Detail", "All variants keep corrected NXT v3.0 exit and swap only the noise/directional filter.");
writeMatrix(grid, "A4", [[
...
baseSheet(assumptions, "Backtest Assumptions", "Noise-filter grid using the project workbook template.");
writeMatrix(assumptions, "A4", [["#", "Assumption"], ...[
  "Only the final noise/directional filter changes between variants.",
  ...
  "Best variant in this grid is RSI14 regime: LONG RSI14 > 50; SHORT RSI14 < 50.",
