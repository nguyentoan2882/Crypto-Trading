...
const outDir = path.resolve("outputs", "nxt_v31_runner_ab_6y");
const jsonPath = path.join(outDir, "nxt_v31_runner_ab_6y_results.json");
const xlsxPath = path.join(outDir, "NXT_V31_RSI50_Runner_AB_6Y_BTC_SOL_SUI_20K.xlsx");
...
const rsi50Filter = {
  key: "rsi_50",
  name: "RSI regime 50",
  description: "LONG RSI14 > 50; SHORT RSI14 < 50",
  pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > 50 : c.rsi14 < 50),
};

const runnerVariants = [
  { key: "runner_a_50_50_ssl", name: "Runner A 50/50 SSL", lockWeight: 0.5, runnerWeight: 0.5, description: "Close 50% at 2.5 ATR; keep 50% runner until SSL opposite flip or breakeven stop." },
  { key: "runner_b_70_30_ssl", name: "Runner B 70/30 SSL", lockWeight: 0.7, runnerWeight: 0.3, description: "Close 70% at 2.5 ATR; keep 30% runner until SSL opposite flip or breakeven stop." },
];
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT v3.1 RSI 50/50 Runner A/B - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | Template: NXT_Backtest_Workbook_Template.xlsx`);
  ...
  baseSheet(assumptions, "Backtest Assumptions", "NXT v3.1 RSI 50/50 runner A/B.");
...
const variants = runnerVariants.map(runner => {
  const trades = symbols.flatMap(symbol => backtestSymbol(symbol, enriched[symbol], weeklyBySymbol[symbol], rsi50Filter, runner));
...
const data = {
  ...
  assumptions: [
    ...
    "Entry logic and RSI 50/50 noise filter are unchanged from the official NXT v3.1 run.",
    ...
    "RSI filter: LONG requires RSI14 > 50; SHORT requires RSI14 < 50.",
