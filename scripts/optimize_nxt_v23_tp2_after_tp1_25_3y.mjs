...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_3y_v23_tp2_grid_tp1_25");
const jsonPath = path.join(outDir, "nxt_v23_tp2_grid_tp1_25_3y_results.json");
const xlsxPath = path.join(outDir, "NXT_V23_TP2_Grid_TP1_25_3Y_BTC_SOL_SUI.xlsx");
...
const fixedTp1Atr = 2.5;
const tp2AtrValues = [2.6, 2.8, 3.0, 3.2, 3.4, 3.6, 3.8, 4.0, 4.2, 4.4, 4.6, 4.8, 5.0];
...
function backtestSymbol(symbol, daily, weekly, tp2Atr) {
  const trades = [];
  let position = null;
  let tradeNo = 1;
  ...
  for (let i = 51; i < daily.length - 1; i++) {
    ...
    position = {
      ...
      stop: initialStop,
      riskPerUnit,
      tp1: side === "LONG" ? entryPrice + c.atr14 * fixedTp1Atr : entryPrice - c.atr14 * fixedTp1Atr,
      ...
      notes: [
        ...
        `TP1 ${fixedTp1Atr.toFixed(1)} x ATR(14); TP2 ${tp2Atr.toFixed(1)} x ATR(14)`,
...
function overallStats(trades, tp2Atr) {
...
async function buildWorkbook(data) {
  ...
  const ranking = workbook.worksheets.add("TP2 Grid");
  baseSheet(ranking, "NXT v2.3 TP2 Grid - BTC SOL SUI 3Y", `${data.period.start} to ${data.period.end} | TP1 fixed at ${fixedTp1Atr} ATR; only TP2 ATR is varied`);
  const rankRows = [[
    "Rank", "TP1 ATR", "TP2 ATR", "Trades", "Wins", "Losses", "Win Rate", "TP1 Hit Rate", "Total R", "Average R / Trade", "Max DD", "Best R", "Worst R", "BTC R", "SOL R", "SUI R",
  ], ...data.optimizationResults.map((r, i) => [
    i + 1, r.tp1Atr, r.tp2Atr, r.trades, r.wins, r.losses, r.winRate, r.tp1HitRate, r.totalR, r.avgR, r.maxDrawdownR, r.bestR, r.worstR,
  ...
  numberFormat(ranking, `G5:H${rankRows.length + 3}`, "0.0%");
  numberFormat(ranking, `I5:P${rankRows.length + 3}`, "0.00");
  ranking.freezePanes.freezeRows(4);
  ranking.getRange(`A4:P${rankRows.length + 3}`).format.wrapText = true;
  finishSheet(ranking, `A1:P${rankRows.length + 3}`);
  ...
  baseSheet(summary, "NXT Trading System v2.3 - Best TP2 Variant", `${data.period.start} to ${data.period.end} | TP1 ${fixedTp1Atr} x ATR, TP2 ${data.best.tp2Atr} x ATR | Source: ${data.source} | Net of fee/slippage model`);
  ...
  baseSheet(trades, "Best TP2 Detailed Trades", "One completed trade per row for the best TP2 setting; TP1 fixed at 2.5 ATR and all other NXT v2.3 rules unchanged.");
  ...
  baseSheet(assumptions, "TP2 Grid Assumptions", "Objective translation of NXT_Trading_System.docx with TP2 sensitivity test.");
...
const runs = tp2AtrValues.map(tp2Atr => {
  const trades = symbols.flatMap(symbol => backtestSymbol(symbol, enriched[symbol], weeklyBySymbol[symbol], tp2Atr));
  trades.sort((a, b) => new Date(a.exitTime) - new Date(b.exitTime));
  return { tp2Atr, trades, stats: overallStats(trades, tp2Atr) };
});
...
const data = {
  ...
  tp2AtrValues,
  best: bestRun.stats,
  optimizationResults: runs.map(r => r.stats),
  summary: summarize(trades),
  trades,
  bySymbol,
  datasets,
  assumptions: [
    ...
    `TP1 is fixed at ${fixedTp1Atr} x ATR(14), closing 50% and moving stop to breakeven.`,
    `TP2 grid tests ${tp2AtrValues[0]} to ${tp2AtrValues[tp2AtrValues.length - 1]} x ATR(14) in 0.2 increments, closing the remaining 50%. Remaining position can also exit on opposite SSL flip, stop, or end-of-test mark-to-market.`,
...
console.log(JSON.stringify({
  ...
  xlsxPath,
  best: data.best,
  topFive: data.optimizationResults.slice(0, 5).map(r => ({
    tp1Atr: r.tp1Atr,
    tp2Atr: r.tp2Atr,
    trades: r.trades,
    winRate: r.winRate,
