...
const outDir = path.resolve("outputs", "nxt_v30_rsi_regime_grid_6y");
const jsonPath = path.join(outDir, "nxt_v30_rsi_regime_grid_6y_results.json");
const xlsxPath = path.join(outDir, "NXT_V30_RSI_Regime_Grid_6Y_BTC_SOL_SUI.xlsx");
...
const filterVariants = rsiBands.map(band => ({
  key: `rsi_${band.long}_${band.short}`,
  name: `RSI ${band.long}/${band.short}`,
  description: `LONG RSI14 > ${band.long}; SHORT RSI14 < ${band.short}`,
  rsiLong: band.long,
  rsiShort: band.short,
  pass: (side, c) => c.rsi14 != null && (side === "LONG" ? c.rsi14 > band.long : c.rsi14 < band.short),
}));
...
const data = {
  ...
  assumptions: [
    ...
    "RSI regime grid keeps the corrected NXT v3 entry/exit structure and varies only RSI14 long/short thresholds.",
    ...
    "Common long entry requires SSL 10/10 bullish crossover, price crossing above EMA20 within the last 3 candles, distance from close to EMA50 <= 2 ATR(14), plus RSI14 above the tested long threshold.",
    "Common short entry requires SSL 10/10 bearish crossover, price crossing below EMA20 within the last 3 candles, distance from close to EMA50 <= 2 ATR(14), plus RSI14 below the tested short threshold.",
    ...
    "Tested RSI threshold pairs: 50/50, 52/48, 55/45, 58/42, and 60/40.",
