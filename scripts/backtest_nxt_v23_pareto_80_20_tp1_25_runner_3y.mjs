...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_3y_v23_pareto_80_20_tp1_25_runner");
const jsonPath = path.join(outDir, "nxt_v23_pareto_80_20_tp1_25_runner_3y_results.json");
const xlsxPath = path.join(outDir, "NXT_V23_Pareto_80_20_TP1_25_Runner_3Y_BTC_SOL_SUI.xlsx");
const feeRate = 0.0006;
const slippageRate = 0.0005;
const roundTripCostRate = 2 * (feeRate + slippageRate);
const tp1Atr = 2.5;
const tp1Weight = 0.8;
const runnerWeight = 1 - tp1Weight;
const trailAtr = 1.5;
const yahooSymbols = {
  BTCUSDT: "BTC-USD",
  SOLUSDT: "SOL-USD",
...
function backtestSymbol(symbol, daily, weekly) {
  ...
  for (let i = 51; i < daily.length - 1; i++) {
    ...
    if (position) {
      ...
      const remainingWeight = position.tp1Done ? runnerWeight : 1;
      ...
      if (side === "LONG") {
        if (c.low <= position.stop) {
          ...
          if (!position.tp1Done && c.high >= position.tp1) {
            ...
            position.notes.push(`TP1 hit ${iso(c.time)}; ${(tp1Weight * 100).toFixed(0)}% closed and runner stop moved to breakeven`);
          }
          if (position.tp1Done && c.time > position.tp1Time) {
            const trailedStop = c.close - c.atr14 * trailAtr;
      ...
      if (exitPrice != null) {
        ...
        trades.push({
          ...
          runnerReference: position.runnerReference,
          exitTime: iso(c.time),
          exitPrice,
          exitReason,
    ...
    position = {
      ...
      runnerReference: side === "LONG" ? entryPrice + c.atr14 * trailAtr : entryPrice - c.atr14 * trailAtr,
      tp1Done: false,
      tp1Time: null,
      realizedR: 0,
      ema20: c.ema20,
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT v2.3 Pareto 80/20 Runner - BTC SOL SUI 3Y", `${data.period.start} to ${data.period.end} | TP1 ${tp1Atr} ATR closes ${(tp1Weight * 100).toFixed(0)}%; ${(runnerWeight * 100).toFixed(0)}% runner trails ${trailAtr} ATR | Source: ${data.source}`);
  ...
  const tradeHeaders = [
    ...
    "TP1", "TP1 Time", "Runner Ref", "Exit Time", "Exit Price", "Exit Reason", "Gross R", "Cost R", "Net R", "% Move", "TP1 Hit",
...
const data = {
  ...
  assumptions: [
    ...
    `Pareto 80/20 variant keeps v2.3 entries, closes ${(tp1Weight * 100).toFixed(0)}% at TP1 = ${tp1Atr} ATR, moves the runner stop to breakeven after TP1, and starts ${trailAtr} ATR trailing from the following daily candle onward.`,
    ...
    `TP1 is ${tp1Atr} x ATR(14), closing ${(tp1Weight * 100).toFixed(0)}% and moving runner stop to breakeven.`,
    `The remaining ${(runnerWeight * 100).toFixed(0)}% is no longer closed at fixed TP2. From the daily candle after TP1 onward, it trails using ${trailAtr} x ATR(14) from each close and exits on opposite SSL flip, trail stop, or end-of-test mark-to-market.`,
