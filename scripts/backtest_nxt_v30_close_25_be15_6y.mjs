...
const outDir = path.resolve("outputs", "nxt_crypto_btc_sol_sui_6y_v30_close_25_be15");
const jsonPath = path.join(outDir, "nxt_v30_close_25_be15_6y_results.json");
const xlsxPath = path.join(outDir, "NXT_V30_Close_25_BE15_6Y_BTC_SOL_SUI.xlsx");
...
async function buildWorkbook(data) {
  ...
  baseSheet(summary, "NXT v3.0 Close 2.5 ATR + BE at 1.5 ATR - BTC SOL SUI 6Y", `${data.period.start} to ${data.period.end} | BE trigger ${breakEvenAtr} ATR | Full exit ${closeAtr} ATR | Source: ${data.source}`);
  ...
  baseSheet(trades, "Detailed Trades", "One completed trade per row; full exit at 2.5 ATR with stop moved to breakeven after 1.5 ATR.");
  ...
  baseSheet(assumptions, "Backtest Assumptions", "NXT v3.0 variant with breakeven trigger at 1.5 ATR.");
...
const data = {
  ...
  assumptions: [
    ...
    `When price reaches ${breakEvenAtr} x ATR(14) in favor of the trade, stop moves to breakeven. No partial profit is taken at this level.`,
    `Full position exits at ${closeAtr} x ATR(14). Position can also exit on opposite SSL flip, stop, breakeven stop, or end-of-test mark-to-market if 2.5 ATR is not reached.`,
